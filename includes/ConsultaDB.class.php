 ...
 #DESCRIPCION: colocamos el status del usuario a Offline
  function UsuarioOffline($IdUsuario) {
    ...
    function ConsultaUsuarios(){
        $dbLuminarias = new database;
        if ($dbLuminarias->conectar() == true) {
            $query = "SELECT * FROM tbl_usuario_login WHERE activo_usuario = 1";
            $result = @mysql_query($query) or die(mysql_error());
            if (!$result)
                return false;
            else
...
INNER JOIN tbl_tipogeneracion ON tbl_tipogeneracion.Pk_TipoGeneracion = cat_generacion.TipoGeneracion
    ...
    function obtenerDirectorios($id)
    {
        $db=new database();
        if($db->conectar()==true){
            $sql="SELECT * FROM cat_directorio_preparatorias WHERE pk_prepa = ".$id;
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
            }else{
...
AND tbl_carreras.pk_carreras = rel_trabajadorecarreras.fk_carreras AND rel_trabajadorecarreras.activoPersona = 1";
  ...
  //                  INNER JOIN cat_estado ON cat_estado.pk_estado = cat_municipios.fk_estado
    ...
    function ConDerechoPromPorAlumno($fk_alumno){
         $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT derechoPromedio FROM `tbl_egresados` where fk_alumnos = $fk_alumno
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function ConCheckTramiteCredencial($fk_alumno){
         $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT estatusCredencial,fechaEntregaCredencial FROM `tbl_egresados` where fk_alumnos = $fk_alumno
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
     ...
     function ConCheckTieneCURP($fk_alumno){
         $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT curp FROM `tbl_alumnos` where pk_alumno = $fk_alumno
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
...
ORDER BY autorizacion1 ASC
	...
	#MONKY------------------------------------------------------------
    function ConCantidadAlumnosEgresadosTrabajando($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT COUNT(*) as cantidadTotalTrabajan
                    FROM tbl_alumnos
    ...
    function ConCantidadAlumnosEgresadosPorCarrera($fk_nivelestudio, $fk_modalidad, $fk_carreras){
        $db =  new database;
        if($db->conectar() == true){
    ...
    function ConCantidadAlumnosNoTituladosContacto($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    matricula,
    ...
    function ConObtenerGeneracionesTodas($fk_nivelestudio, $fk_modalidad, $fk_carreras){
        ...
        if($db->conectar() == true){
            $sql = "SELECT
                    ...
                    INNER JOIN cat_anios ON cat_anios.pk_anios = tbl_alumnos.PlanEstudios
                    INNER JOIN cat_generacion ON cat_generacion.pk_generacion = tbl_alumnos.fk_generacion
                    INNER JOIN cat_anios b ON b.pk_anios = cat_generacion.fk_finanios
                    WHERE
                    tbl_alumnos.fk_nivelestudio = '$fk_nivelestudio' AND
                    tbl_alumnos.fk_modalidad = '$fk_modalidad' AND
                    tbl_alumnos.fk_carreras = '$fk_carreras'
    ...
    function ConObtenerGeneracionPlanEstudios($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                SELECT * FROM tbl_gen_plan_group tgpg
                WHERE
                tgpg.fk_nivelestudio = '$fk_nivelestudio'
                AND tgpg.fk_modalidad = '$fk_modalidad'
                AND tgpg.fk_carrera = '$fk_carreras'
                AND tgpg.fk_generacion = '$fk_generacion'
                ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function ConObtenerGeneracionesTodasTop2015($fk_nivelestudio, $fk_modalidad, $fk_carreras){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    cat_generacion.descripcion as DescripcionGeneracion,
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras'
                    GROUP BY tbl_alumnos.generacionNumero+0
                    HAVING (cat_generacion.descripcion LIKE '%2015%')
                    OR (cat_generacion.descripcion LIKE '%2016%')
                    OR (cat_generacion.descripcion LIKE '%2017%')
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    //para la tabla de egresados obtiene el numero de registro de las generaciones (MONKYYYYYYYYYY)
    //con todas la generaciones por carreras (lic hugo)
    function ConObtenerNoGeneracionesTodas($fk_nivelestudio, $fk_modalidad, $fk_carreras){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    ...
                    tbl_alumnos.fk_modalidad = '$fk_modalidad' AND
                    tbl_alumnos.fk_carreras = '$fk_carreras'
                     GROUP BY tbl_alumnos.generacionNumero+0 ) AS Result
					";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	 ...
	 function ConObtenerGeneracionesCinco($fk_nivelestudio, $fk_modalidad, $fk_carreras, $limit){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    cat_generacion.descripcion as DescripcionGeneracion,
                    ...
                    tbl_alumnos.generacionNumero,
                    tbl_alumnos.fk_generacion,
                    tbl_gen_plan_group.desc_planestudios
                    FROM tbl_alumnos
                    ...
                    INNER JOIN cat_anios ON cat_anios.pk_anios = tbl_alumnos.PlanEstudios
                    INNER JOIN cat_generacion ON cat_generacion.pk_generacion = tbl_alumnos.fk_generacion
                    LEFT JOIN tbl_gen_plan_group ON tbl_gen_plan_group.fk_generacion = cat_generacion.pk_generacion
                    WHERE
                    tbl_alumnos.fk_nivelestudio = '$fk_nivelestudio' AND
                    tbl_alumnos.fk_modalidad = '$fk_modalidad' AND
                    tbl_alumnos.fk_carreras = '$fk_carreras'
    ...
    function ConReporteSinodalesPago($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fechaUnoPago, $fechaDosPago){
        ...
        if($db->conectar() == true){
            $sql = "SELECT *,
                    ...
                    ORDER BY tbl_tomadeprotesta.FechaTomaProtesta, fk_carreras ASC
...
# @return        True ó False en función de la consulta
 ...
 #DESCRIPCION: colocamos el status del usuario a online
  function ConExisteTrabajadorUsuario($pk_trabajador) {
    ...
    //para la tabla de egresados
    function ConCantidadEgresadosTituladosPorAnio($fk_nivelestudio, $fk_modalidad, $fk_carreras, $anio, $genero){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                   COUNT(*) AS TotalTitu
                    ...
                    tbl_alumnos.fk_modalidad = '$fk_modalidad' AND
                    tbl_alumnos.fk_carreras = '$fk_carreras'  AND
                    tbl_tomadeprotesta.FechaTomaProtesta BETWEEN '" . $anio . "-01-01' and '" . $anio . "-12-31' AND
                    fk_estadoTitulacion='1' AND tbl_alumnos.fk_genero IN (".$genero.")
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function ConCantidadEgresadosTituladosPorAnioHombres($fk_nivelestudio, $fk_modalidad, $fk_carreras, $anio){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                   COUNT(*) AS TotalHo
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras'  AND
                    tbl_tomadeprotesta.FechaTomaProtesta BETWEEN '" . $anio . "-01-01' and '" . $anio . "-12-31' AND
                    fk_estadoTitulacion='1' AND tbl_alumnos.fk_genero='1'
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function ConCantidadEgresadosTituladosPorAnioMujeres($fk_nivelestudio, $fk_modalidad, $fk_carreras, $anio){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                   COUNT(*) AS TotalMu
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras'  AND
                    tbl_tomadeprotesta.FechaTomaProtesta BETWEEN '" . $anio . "-01-01' and '" . $anio . "-12-31' AND
                    fk_estadoTitulacion='1' AND tbl_alumnos.fk_genero='2'
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    //con todas la generaciones por carreras (lic hugo)
    function ConReporteAlumnosTituloExpedidoSoloTitulados($fk_nivelestudio, $fk_modalidad, $fk_carreras,$fechaInicio, $fechaFin){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    cat_generacion.descripcion as DescripcionGeneracion,
    ...
    function ConReporteAlumnosTituladosByFecha($fk_nivelestudio, $fk_modalidad, $fk_carreras,$fechaInicio, $fechaFin){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    *
...
AND fk_nivelestudio='$fk_nivelestudio' AND fk_modalidad='$fk_modalidad'";
	...
	function MioConCantidadTotalAlumnosTramites($fk_nivelestudio, $fk_modalidad, $fk_carreras){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT *
                    FROM tbl_alumnos
	...
	function MioConCantidadTotalAlumnosByGenTramites($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT *
                    FROM tbl_alumnos
                    ...
                    INNER JOIN tbl_carreras ON tbl_carreras.pk_carreras = tbl_alumnos.fk_carreras
                    WHERE  tbl_carreras.fk_nivelestudio = '$fk_nivelestudio'
                    AND tbl_carreras.fk_modalidad = '$fk_modalidad'
                    AND pk_carreras='$fk_carreras'
                    AND fk_generacion = '$fk_generacion' ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
            }else{
    ...
    function MioConCantidadTotalAlumnosByGenTramitesTimeJob($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion, $tipo){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT COUNT(*) AS cuantosEncuentranJob
                    FROM tbl_alumnos
                    LEFT JOIN tbl_egresados ON tbl_egresados.fk_alumnos = tbl_alumnos.pk_alumno
                    INNER JOIN cat_genero ON cat_genero.pk_genero = tbl_alumnos.fk_genero
                    INNER JOIN tbl_carreras ON tbl_carreras.pk_carreras = tbl_alumnos.fk_carreras
...
LEFT JOIN tbl_tomadeprotesta ON tbl_tomadeprotesta.fk_alumno = tbl_alumnos.pk_alumno
    ...
    function ConReporteAlumnosInformacionBasica($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion, $edoTitulacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
		    ...
		    cat_anios.descripcion as DescripcionPlan, tbl_alumnos.curp
		    ...
		    LEFT JOIN cat_genero ON cat_genero.pk_genero = tbl_alumnos.fk_genero
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras' AND
                    tbl_alumnos.fk_generacion='$fk_generacion' AND
                    tbl_egresados.fk_estadoTitulacion='$edoTitulacion'
                    order by apaterno
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
	    ...
	    function ConReporteAlumnosInformacionBasicaGeneracion($fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    cat_generacion.descripcion as DescripcionGeneracion
 ...
 //informacion personal
    ...
    function ConReporteAlumnosCredencialesTramitadasPorGeneracion($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        ...
        if($db->conectar() == true){
            $sql = "SELECT
					tbl_egresados.estatusCredencial as estatusCred,
                    DATE_FORMAT(tbl_egresados.fechaEntregaCredencial, '%d/%m/%Y') as fechEntregaCred,
                    cat_generacion.descripcion as DescripcionGeneracion,
                    cat_turnos.descripcion as DescripcionTurnos, fk_estadoTitulacion,
                     cat_generacion.descripcion as DescripcionGeneracion,
                    cat_opciontitulacion.Nombre as NombreOpcionTitulacion,
    ...
    INNER JOIN cat_colonia ON cat_colonia.pk_colonia = tbl_alumnos.fk_colonia
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras' AND
                    tbl_alumnos.fk_generacion='$fk_generacion'
					order by tbl_egresados.fechaEntregaCredencial ASC
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
...
//********************************BOTONES VERDES DE EGRESADOS******************************************//
    ...
    function ConCantidadEgresadosEdadYGenero($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion, $edad, $genero){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras' AND
                    tbl_alumnos.fk_generacion = '$fk_generacion' AND
                    tbl_egresados.edadEgreso ".$edad." AND
                    tbl_alumnos.fk_genero = '".$genero."'
                    ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
...
#DESCRIPCION: BOTON REPORTES TITULADOS POR EDAD
    ...
    function ConCantidadTituladosEdadYGenero($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion, $edad, $genero){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    COUNT(*) as cantidadTotal
...
";
	...
	#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx MONO xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx


	function CantidadAlumnosEncuestaMedicinaGraficaInstitucionesLaboran(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT descripcion_institucioneslabora,
						COUNT(descripcion_institucioneslabora) AS cantidad
						FROM tbl_encuestamedicina
						INNER JOIN cat_institucioneslabora
						ON tbl_encuestamedicina.fk_institucioneslabora=cat_institucioneslabora.descripcion_institucioneslabora
						GROUP BY descripcion_institucioneslabora
					   ";
                    $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	...
	function CantidadAlumnosEncuestaMedicinaGraficaPerteneceOrganizacion(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT
						IF(PerteneceOrganizacionSocial = 1,'SI','NO') AS Legend,
						COUNT(*) AS cantidad
						FROM tbl_encuestamedicina
						GROUP BY PerteneceOrganizacionSocial
					   ";
	...
	function CantidadAlumnosEncuestaMedicinaGraficaCertificadosProfecionalmente(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT
						IF(CertificacionProfesional = 1,'SI','NO') AS Legend,
						COUNT(*) AS cantidad
						FROM tbl_encuestamedicina
						GROUP BY CertificacionProfesional
					   ";
	...
	function CantidadAlumnosEncuestaMedicinaGraficaCapacitadosLaboralmente(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT
						IF(CapacitacionTrabajoActual = 1,'SI','NO') AS Legend,
						COUNT(*) AS cantidad
						FROM tbl_encuestamedicina
						GROUP BY CapacitacionTrabajoActual
					   ";
                    $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	...
	function CantidadAlumnosEncuestaMedicinaGraficaUsoCienciasBasicas(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT GradoCienciasBasicas,
						COUNT(*) AS cantidad
						FROM tbl_encuestamedicina
						GROUP BY GradoCienciasBasicas
					   ";
                    $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	...
	function CantidadAlumnosEncuestaMedicinaGraficaUsoCienciasClinicas(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT GradoCienciasClinicas,
						COUNT(*) AS cantidad
						FROM tbl_encuestamedicina
						GROUP BY GradoCienciasClinicas
					   ";
                    $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	...
	function CantidadAlumnosEncuestaMedicinaGraficaAspectoDebilidad(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT descripcion_aspectodebilidad,
						COUNT(*) AS cantidad
						FROM tbl_encuestamedicina
						INNER JOIN cat_aspectodebilidad
						ON descripcion_aspectodebilidad = fk_aspectodebilidad
	...
	function CantidadAlumnosEncuestaMedicinaGraficaReeleccionInstituto(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
						SELECT
						IF(ElegirMismaInstitucion = 1,'SI','NO') AS Legend,
						COUNT(*) AS cantidad
						FROM tbl_encuestamedicina
						GROUP BY ElegirMismaInstitucion
					   ";
                    $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function busquedaPorParametrosMuertos($opcion){
        $db = new database;
        if ($db->conectar() == true) {
            $sql = "
                SELECT a.*,c.nombreCarrera,e.fk_estadoTitulacion,e.estatusCredencial
                FROM tbl_alumnos a
                INNER JOIN tbl_carreras c ON c.pk_carreras = a.fk_carreras
                INNER JOIN tbl_egresados e ON e.fk_alumnos = a.pk_alumno
                WHERE a.Activo_Alumno = 2
     ...
     function CantidadAlumnosEncuestaMedicinaGraficaGradoSatisfaccion(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
                        SELECT fk_gradosatisfaccion,
                        COUNT(*) AS cantidad
                        FROM tbl_encuestamedicina
                        GROUP BY fk_gradosatisfaccion
                       ";
                    $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
 ...
 /************************************************************************************************************/
//***********************************G R A F I C A S     D  E     E N C U E S T A S   2 0 1 9 **************************//


    function Encuestas2019MisionVisionValores(){
        $db =  new database;
        if($db->conectar() == true){
                $sql = "
                        SELECT
                        tbl_encuestaEmpleadores2019 
                        COUNT(*) 
                        FROM tbl_encuestaEmpleadores2019
                       ";
                    $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
...
function verCredencialesEgresadosAnuales($busquedaAnhio,$mes,$car){
		$db = new database();
		if ($db->conectar() == true) {
		/*$query = "SELECT COUNT(pk_carreras) AS totalbycar, fk_alumnos,estatusCredencial,fechaEntregaCredencial,pk_alumno,fk_carreras,pk_carreras,nombreCarrera FROM `tbl_egresados`
		INNER JOIN tbl_alumnos ON pk_alumno=fk_alumnos
		INNER JOIN tbl_carreras ON pk_carreras=fk_carreras
		WHERE estatusCredencial=1 AND pk_carreras='$flagMes'
		AND (fechaEntregaCredencial BETWEEN '2013-01-01' AND '2013-12-31') GROUP BY DATE_FORMAT(fechaEntregaCredencial,'%m')"*/
		$query= "SELECT COUNT(pk_carreras) AS totalbycar,
		fk_alumnos,estatusCredencial,fechaEntregaCredencial,pk_alumno,fk_carreras,pk_carreras,nombreCarrera
		FROM tbl_egresados
		INNER JOIN tbl_alumnos ON pk_alumno=fk_alumnos
		INNER JOIN tbl_carreras ON pk_carreras=fk_carreras
		WHERE estatusCredencial=1 AND pk_carreras='$car'
		AND MONTH(fechaEntregaCredencial)='$mes' AND YEAR(fechaEntregaCredencial)='$busquedaAnhio'";
		$result = @mysql_query($query) or die(mysql_error());
			if(!$result){
				return false;
			}else{
...
function verCredencialesEgresadosTotalAnuales($ano,$car){
        $db = new database();
        if ($db->conectar() == true) {
        $query= "
                SELECT COUNT(*) AS totalbycar,
                fk_alumnos,estatusCredencial,fechaEntregaCredencial,pk_alumno,fk_carreras,pk_carreras,nombreCarrera
                FROM tbl_egresados
                INNER JOIN tbl_alumnos ON pk_alumno=fk_alumnos
                INNER JOIN tbl_carreras ON pk_carreras=fk_carreras
                WHERE estatusCredencial=1
                AND pk_carreras='$car'
                AND YEAR(fechaEntregaCredencial)='$ano'
                ";
        $result = @mysql_query($query) or die(mysql_error());
            if(!$result){
                return false;
	...
	function CarrerasActivasLicenciaturas($fk_nivelestudio, $fk_modalidad){
		$db = new database();
		if ($db->conectar() == true) {
			$query="SELECT pk_carreras,fk_nivelestudio,fk_modalidad,nombreCarrera FROM tbl_carreras
					WHERE estadoCarrera=1
					AND fk_nivelestudio ='$fk_nivelestudio'
					AND fk_modalidad ='$fk_modalidad'";
			$result = @mysql_query($query) or die(mysql_error());
			if(!$result){
				return false;
	...
	function CarrerasActivasLicenciaturasbyPk($pk_carreras){
		$db = new database();
		if ($db->conectar() == true) {
			$query="SELECT pk_carreras,fk_nivelestudio,nombreCarrera FROM tbl_carreras WHERE pk_carreras='$pk_carreras' AND estadoCarrera=1 AND fk_nivelestudio =2";
			$result = @mysql_query($query) or die(mysql_error());
			if(!$result){
				return false;
			}else{
	...
	function EgresadosEmpresasEmpleadores($fk_nivelestudio,$fk_modalidad,$fk_carreras){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
					SELECT pk_alumno,matricula,
					CONCAT_WS( ' ', tbl_alumnos.apaterno , tbl_alumnos.amaterno , tbl_alumnos.nombre ) AS NombreCompleto,
					IF(fk_genero=1,'HOMBRE','MUJER') AS Sexo,
					fk_carreras,tbl_alumnos.fk_nivelestudio AS nivelAlumno,tbl_carreras.fk_nivelestudio AS nivelCarrera,tbl_alumnos.fk_modalidad,
					fk_alumnos,estatusTrabajo,
					IF(nombreEmpresaTrabajo=' ', 'NO SE REGISTRÒ', nombreEmpresaTrabajo) AS nombreEmpresaTrabajo,
					IF(puestoTrabajo=' ', 'NO ASIGNADO', puestoTrabajo) AS puestoTrabajo,direccionTrabajo,
					IF(telefonoTrabajo=' ', 'NO REGISTRADO', telefonoTrabajo) AS telefonoTrabajo,pk_carreras,nombreCarrera
					FROM tbl_alumnos
					INNER JOIN tbl_egresados ON tbl_egresados.fk_alumnos = tbl_alumnos.pk_alumno
					INNER JOIN cat_genero ON cat_genero.pk_genero = tbl_alumnos.fk_genero
					INNER JOIN tbl_carreras ON tbl_carreras.pk_carreras = tbl_alumnos.fk_carreras
					WHERE
					tbl_alumnos.fk_nivelestudio ='$fk_nivelestudio'
					AND tbl_alumnos.fk_carreras ='$fk_carreras'
					AND tbl_alumnos.fk_modalidad ='$fk_modalidad'
					AND tbl_egresados.estatusTrabajo = '1'
				   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	...
	function filtroAlumonosEgresadosError(){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
					...
					LEFT JOIN tbl_egresados ON tbl_egresados.fk_alumnos = tbl_alumnos.pk_alumno
					LEFT JOIN tbl_tomadeprotesta ON tbl_tomadeprotesta.fk_alumno = tbl_alumnos.pk_alumno
					WHERE fk_carreras =74
					AND fk_generacion =265
				   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	...
	#MOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOONKY
	function ObtieneTodasLasGeneracionesHaciendoCorte($fk_nivelestudio, $fk_modalidad, $fk_carreras, $Generacioncorte){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT *,
					COUNT(*) as EgresadosByCarrera,
                    cat_generacion.descripcion as DescripcionGeneracion,
                    cat_turnos.descripcion as DescripcionTurnos,
                    tbl_alumnos.generacionNumero,
                    tbl_alumnos.fk_generacion
                    ...
                    LEFT JOIN tbl_egresados ON tbl_egresados.fk_alumnos = tbl_alumnos.pk_alumno
                    LEFT JOIN tbl_tomadeprotesta on tbl_egresados.fk_alumnos=tbl_tomadeprotesta.fk_alumno
                    ...
                    tbl_alumnos.fk_modalidad = '$fk_modalidad' AND
                    tbl_alumnos.fk_carreras = '$fk_carreras'
                    GROUP BY tbl_alumnos.generacionNumero
                    HAVING tbl_alumnos.generacionNumero <=".$Generacioncorte."
                    ORDER BY CAST(tbl_alumnos.generacionNumero as unsigned)
				   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
	...
	function ObtieneTodasLasGeneracionesDespuesDeCorte($fk_nivelestudio, $fk_modalidad, $fk_carreras, $Generacioncorte){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT *,
					COUNT(*) as EgresadosByCarrera,
	...
	function ObtieneLaPKdelAlumno($matricula){
		 $db =  new database;
        if($db->conectar() == true){
			$sql="SELECT pk_alumno FROM tbl_alumnos WHERE matricula='".$matricula."'";
			$result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
            }else{
	...
	function MaxPrimarykeyTable(){
	$db =  new database;
        if($db->conectar() == true){
			$sql="SELECT MAX(pk_alumno) AS ultimoID FROM tbl_alumnos";
			$result = @mysql_query($sql) or die(mysql_error());
    ...
    function ConsultaDatosAlumnosParaTablaDeBusqueda(){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT
                    pk_alumno,
                    matricula,
                    pk_carreras,
                    CONCAT(apaterno,' ',amaterno,' ',nombre) AS Alumno,
                    nombreCarrera
                    FROM tbl_alumnos
                    INNER JOIN tbl_carreras ON pk_carreras = fk_carreras";
            $result = @mysql_query($sql) or die(mysql_error());
    ...
    function ConsultaTimbreQuemado($folio_timbre){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "SELECT *
                    FROM tbl_timbres_secretaria TTS
                    INNER JOIN tbl_alumnos TAL ON TAL.pk_alumno = TTS.fk_alumno
                    INNER JOIN cat_doctos_oficiales CDO ON CDO.pk_docto = TTS.fk_documento
                    WHERE folio_timbre = '".$folio_timbre."'";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
            }else{
    ...
    function ConCantidadAlumnosEgresadosConMaestria($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                   SELECT COUNT(*) as cantidadTotalEnMaestria
                    FROM tbl_alumnos
    ...
    function ConCantidadAlumnosEgresadosConDoctorado($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                   SELECT COUNT(*) as cantidadTotalEnDoctorado
                    FROM tbl_alumnos
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras' AND
                    tbl_alumnos.fk_generacion = '$fk_generacion' AND
                    tbl_egresados.doctoradoNombre != '' AND
                    tbl_egresados.fk_estadoTitulacion= 1
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
    ...
    function ConCantidadAlumnosEgresadosConEspecialidad($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                   SELECT COUNT(*) as cantidadTotalEnEspecialidad
                    FROM tbl_alumnos
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras' AND
                    tbl_alumnos.fk_generacion = '$fk_generacion' AND
                    tbl_egresados.especialidadNombre != '' AND
                    tbl_egresados.fk_estadoTitulacion= 1
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
    ...
    function ConCantidadAlumnosEgresadosConPosgradoIESCH($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion, $type){
        $db =  new database;
        if($db->conectar() == true){
            if($type==1) $nv='mtria'; elseif($type==1) $nv='doctorado'; else $nv='especialidad';
            $sql = "
                   SELECT COUNT(*) as cantidadPosgradoIesch
                    FROM tbl_alumnos
                    ...
                    tbl_alumnos.fk_carreras = '$fk_carreras' AND
                    tbl_alumnos.fk_generacion = '$fk_generacion' AND
                    tbl_egresados.".$nv."Institucion LIKE '%IESCH%'|| '%Salazar%' || '%Instituto de Estudios Superiores de Chiapas%' AND
                    tbl_egresados.fk_estadoTitulacion= 1
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
    ...
    function ConCantidadAlumnosEgresadosElegirMismaInstitucion($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT
                    IF(nuevaUno = 1,'SI','NO') AS Legend,
                    COUNT(*) AS cantidad
                    FROM tbl_egresados e
                    INNER JOIN tbl_alumnos a ON a.pk_alumno = e.fk_alumnos
                    WHERE a.fk_nivelestudio=$fk_nivelestudio
                    AND a.fk_modalidad=$fk_modalidad
                    AND a.fk_carreras=$fk_carreras
                    AND a.fk_generacion=$fk_generacion
                    AND (nuevaUno != '' OR nuevaUno != 0 OR nuevaUno != NULL)
                    GROUP BY nuevaUno
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
    ...
    function ConOpinionAlumnosEgresadosElegirMismaInstitucion($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT
                    ...
                    AND a.fk_carreras=$fk_carreras
                    AND a.fk_generacion=$fk_generacion
                    AND (porqueUno != '' OR porqueUno != NULL)
                    ORDER BY nuevaUno DESC
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function ConCantidadAlumnosEgresadosEstudiarMaestriaMismaInstitucion($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT
                    IF(nuevaDos = 1,'SI','NO') AS Legend,
                    COUNT(*) AS cantidad
                    FROM tbl_egresados e
                    INNER JOIN tbl_alumnos a ON a.pk_alumno = e.fk_alumnos
                    WHERE a.fk_nivelestudio=$fk_nivelestudio
                    ...
                    AND a.fk_carreras=$fk_carreras
                    AND a.fk_generacion=$fk_generacion
                    AND (nuevaDos != '' OR nuevaDos != 0 OR nuevaDos != NULL)
                    GROUP BY nuevaDos
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
        ...
        function ConOpinionAlumnosEgresadosSugerenciaProgramaEducativo($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT
                    ...
                    AND a.fk_carreras=$fk_carreras
                    AND a.fk_generacion=$fk_generacion
                    AND (sugerencias != '' OR sugerencias != NULL)
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function ConOpinionAlumnosEgresadosEstudiarMaestriaMismaInstitucion($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT
                    ...
                    AND a.fk_carreras=$fk_carreras
                    AND a.fk_generacion=$fk_generacion
                    AND (porqueDos != '' OR porqueDos != NULL)
                    ORDER BY nuevaDos DESC
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
    ...
    function ConDatosCompletosCarreras($fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
            $sql = "
                    SELECT c.nombreCarrera,m.nombreMod,g.descripcion
                    FROM tbl_carreras c,cat_modalidad m,cat_generacion g
                    WHERE c.pk_carreras = $fk_carreras
                    AND m.pk_modalidad = $fk_modalidad
                    AND g.pk_generacion = $fk_generacion
                   ";
            $result = @mysql_query($sql) or die(mysql_error());
            if(!$result){
                return false;
     ...
     //para la tabla de egresados con el reporte de todos los alumnos gresados con credenciales tramitadas
    //con todas la generaciones por carreras (lic hugo)
    function ConReporteAlumnosCredencialesTramitadasPorGeneracion($fk_nivelestudio, $fk_modalidad, $fk_carreras, $fk_generacion){
        $db =  new database;
        if($db->conectar() == true){
    ...
    INNER JOIN cat_colonia ON cat_colonia.pk_colonia = tbl_alumnos.fk_colonia
		...
		#vengo a traer el resulado de la busqueda-------------------------------------------------------------------------------
		function showLastUpdateByUsers($pk_alumno){
			$dbPeon = new database();
			if ($dbPeon->conectar() == true) {
				$query = "SELECT last_upd FROM tbl_egresados WHERE fk_alumnos='$pk_alumno'";
				$result = @mysql_query($query) or die(mysql_error());
				$row = @mysql_fetch_assoc($result);
					if ($row['last_upd']==0){
						return '0|No sea Actualizado';
					}
					else{
						return '1|'.$row['last_upd'];
					}
			}
		}
