...
$texcolz = '3';
	...
	while($prints < count($pkAlu)){
        ...
        $result = $Obras->ConReporteDocumentacionAlumnosPorLlavePrimaria($pk_alumno); //obtenemos todos los datos del alumno
      	if($result){
            ...
            /*if($folio != $row['noactatitulo']){
                if(!$Obras->validarFolioTitulo($pk_alumno, $folio, $row['fk_nivelestudio'])){
                     if($row['noactatitulo'] == '' || $row['noactatitulo'] == '00000' || $row['noactatitulo'] == null){
                          $rs = $Obras->updateFolio($row['pk_egresados'], $folio);
                           if($rs){
                              $bandera = true;
                           }else{
                              $bandera = false;
            ...
            if($bandera){*/
        	   ...
        	   // NOMBE DEL TITULO, VALIDAMOS LA CANTIDAD DE LOS CARACTERES PARA CONOCER SU TAMAÑO
	            ...
	            //$nombreAlumno = utf8_decode(trim($row['NombreAlumno']).' '.trim($row['ApaternoAlumno']).' '.trim($row['AmaternoAlumno']));
              ...
              $nombreAlumno = utf8_decode($nombre.' '.$apaterno.' '.$amaterno);
                ...
                $nomTesis = $Funciones->Capital($nomTesis);
                 if($NombreOpcionTitulacion == "Tesis Profesional") $NombreOpcionTitulacion = $NombreOpcionTitulacion." ".trim($nomTesis); 
            	///////////////////////////////////
                ...
                //contenido o cuerpo
                      ...
                      $pdf->SetXY(20, 98);
                      $pdf->Cell(0,6,utf8_decode('El día '.$Funciones->Fecha2($row['FechaTomaProtesta']).'.'),0,0,'J');
                      $pdf->Ln(20); 
					  ...
					  /*$pdf->SetXY(20, 325);
					  $pdf->Cell(15, 4,utf8_decode('Duplicado'),0, 0, 'C');*/

            
          /* }else{
                $pdf->SetFont('Times','','15');    
                $pdf->Cell(0,7,utf8_decode("Ups!, ocurrio un error."),0,1,'C');
                $pdf->Cell(0,7,utf8_decode("Asegurate que estas asignando el folio correcto,"),0,1,'C');
                $pdf->Cell(0,7,utf8_decode("este folio ya esta asignado o este alumno ya cuenta con un folio."),0,1,'C');
                $pdf->Cell(0,7,utf8_decode("Verifica con el administrador del sistema."),0,1,'C');
                $pdf->Ln(5);
           }           */
      }
	    else{
  ...
  $pdf->Output('Titulos_Atras_licenciatura.pdf','I');
